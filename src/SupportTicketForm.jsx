...
export default function SupportTicketForm() {
  ...
  const [screens, setScreens] = useState([{ name: '', description: '', otherDescription: '', photo: null }])
  ...
  const addScreen = () => {
    setScreens([...screens, { name: '', description: '', otherDescription: '', photo: null }])
  }
  ...
  if (ticketSubmitted) {
    return (
      <Center py={20} bg="#fb6520" minH="100vh">
        <VStack spacing={6}>
          <Text fontSize="3xl" fontWeight="bold" color="white">✅ Ticket Submitted</Text>
          {photosUploading && (
            <HStack spacing={2}>
          ...
          <Button colorScheme="blue" onClick={() => window.location.reload()} isDisabled={photosUploading}>Submit Another</Button>
          <Button variant="outline" onClick={() => navigate('/')} isDisabled={photosUploading}>Close Portal</Button>
        </VStack>
      </Center>
    )
  }
    ...
    <>
        ...
        <Box maxW="600px" w="100%" p={8} bg="black" boxShadow="lg" borderRadius="lg">
          <form onSubmit={handleSubmit}>
            <VStack spacing={5} align="stretch" color="white">
              <FormControl>
                <FormLabel><Icon as={FiMonitor} mr={2} />Store Name</FormLabel>
                <Input {...inputProps} value={storeName} onChange={(e) => setStoreName(e.target.value)} placeholder="Enter store name" isRequired />
              </FormControl>
              ...
              {screens.map((screen, idx) => (
                <Box key={idx} p={4} borderWidth="1px" borderRadius="md" borderColor="#333" bg="#111">
                  <HStack justify="space-between" mb={2}>
                    <Text fontWeight="semibold">Screen {idx + 1}</Text>
                    {screens.length > 1 && (
                      <Button size="xs" colorScheme="red" onClick={() => removeScreen(idx)}><Icon as={FiTrash2} /></Button>
                    )}
                  </HStack>

                  <FormControl mb={3}>
                    <FormLabel>Screen Location</FormLabel>
                    <Input {...inputProps} value={screen.name} onChange={(e) => handleScreenChange(idx, 'name', e.target.value)} placeholder="e.g. Entrance Left" isRequired />
                  </FormControl>

                  <FormControl mb={3}>
                    <FormLabel>Issue Description</FormLabel>
                    <Select {...descriptionInputProps} value={screen.description} onChange={(e) => handleScreenChange(idx, 'description', e.target.value)} placeholder="Select issue" isRequired>
                      {DESCRIPTION_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
                    </Select>
                    {screen.description === 'Other' && (
                      <Textarea {...descriptionInputProps} mt={2} placeholder="Describe issue…" value={screen.otherDescription} onChange={(e) => handleScreenChange(idx, 'otherDescription', e.target.value)} isRequired />
                    )}
                  </FormControl>

                  <FormControl>
                    <FormLabel><Icon as={FiUpload} mr={2} />Photo</FormLabel>
                    <Input type="file" accept="image/*" capture="environment" onChange={(e) => handlePhotoChange(idx, e.target.files[0])} {...inputProps} />
                  </FormControl>
                </Box>
              ))}
              ...
              <FormControl><FormLabel><Icon as={FiUser} mr={2} />Your Name</FormLabel>
                <Input {...inputProps} value={contactName} onChange={(e) => setContactName(e.target.value)} placeholder="Full name" isRequired />
              </FormControl>

              <FormControl><FormLabel><Icon as={FiPhone} mr={2} />Contact Number</FormLabel>
                <Input {...inputProps} value={contactNumber} onChange={(e) => setContactNumber(e.target.value)} placeholder="Phone number" isRequired />
              </FormControl>

              <FormControl><FormLabel><Icon as={FiMail} mr={2} />Email Address</FormLabel>
                <Input {...inputProps} type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="Email address" isRequired />
              </FormControl>

              <Button type="submit" colorScheme="green" size="lg" mt={2} isFullWidth>Submit Ticket</Button>
            </VStack>
          </form>
        </Box>
      </Center>
      ...
      <Modal isOpen={isOpen} onClose={() => {}} isCentered closeOnOverlayClick={false} closeOnEsc={false}>
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}><Progress size="xs" isIndeterminate /></ModalBody>
        </ModalContent>
      </Modal>
    </>
  )
